...
| `setup-dbt-all-tests.sh` + `MANIFEST.dbt-all-tests.md` | Reproduce/verify **every dbt test type driven from `spec/`** (generic, generic-with-args, parameterised package, model-level, singular, unit) and their automatic surfacing as Dagster asset checks ([ADR-0021](../docs/adr/ADR-0021-orchestrator-transform-dq-boundary.md)). Records the package decisions (godatadriven vs calogica `dbt_date`; elementary/automate_dv opt-in; `sqlbuild` is a CLI, not a package). | `just -f .tmp/Justfile verify-dbt-all-tests` (read-only: no dbt run, no warehouse writes) | Claude `claude-opus-4-8`, session `f8f5d90a` · 2026-07-19 |
