...
- **Relates to:** [ADR-0015](ADR-0015-dagster-alt-orchestration-dg-components.md) (Dagster alt-orchestration),
  [ADR-0005](ADR-0005-spec-ssot-bruin-odcs-compiler.md) (spec SSoT + compiler),
  [ADR-0020](ADR-0020-dbt-bruin-primary-transform-engines.md) (dbt primary + Bruin co-primary transform engines)
...
- The check vocabulary must never fail silently. The fail-loud gate is `to_sqlmesh` (ODTS §5),
  which runs over the same spec and rejects unknown check names; `to_dbt` may then skip a check
  that is not projectable for its target flavor (e.g. package tests under `with_packages=False`),
  because the vocabulary was already validated.
...
- **+** Fixed a latent bug the audit exposed: the compiler's check map knew only `not_null`/`unique`,
  ...
  `dbt_utils.accepted_range(min_value=0)`, and the vocabulary is gated fail-loud in `to_sqlmesh`.
...
- The FS row-count assertion that the removed check also made is retained as the final gate in
  `experimental/orchestration/dagster_ogip/e2e/run_combo.sh`; the table-level not-empty test has
  since landed in spec (`checks: [{name: not_empty}]` on `fs.market_features` →
