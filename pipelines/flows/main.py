...
Each SQL engine has its OWN separated Prefect sub-project under `pipelines/<engine>/` (A12
...
deployment that imports the default flow keep working unchanged. `src/ogip/tasks/integrations.py`
shells `python -m pipelines.flows.main` — this module MUST stay importable at this exact path.
...
from pipelines.sqlmesh.flow import flow as ingest_transform_publish
