#!/usr/bin/env bash
# check-no-dagster-dq.sh — enforce ADR-0021: no hand-written Dagster asset_check that duplicates
...
while IFS= read -r hit; do
  ...
  log "VIOLATION ${file#"$REPO"/}:$line — hand-written Dagster asset_check (see ADR-0021)"
...
log "clean — no hand-written Dagster DQ (ADR-0021 satisfied)"
